...
class Vehicle(Object):
    def __init__(
        ...
        v_max: float = 8,
        gamma: float = 5e-7,
        tau_throttle: float = 1,
        tau_steering: float = 1,
    ):
        ...
        """ Current state """
        self.X = np.array(
            [[self.originVCF[0]], [self.originVCF[1]], [0], [0]]
        )  # State vector
        ...
        self.dt = dt
        self.tau_throttle = (
            tau_throttle  # Time constant throttle (Accelerating, not breaking)
        )
    ...
    def one_step_algorithm(self, alpha_ref, v_ref):
        ...
        B_k = np.array([[0], [0], [np.cos(self.heading)], [np.sin(self.heading)]])
        ######################
        # Step 2: Choose A_k #
        ######################
            ...
            A_k = np.array(
                [
                    [
                        1,
                        0,
                        np.sin(self.omega * self.dt) / self.omega,
                        -(1 - np.cos(self.omega * self.dt)) / self.omega,
                    ],
                    [
                        0,
                        1,
                    ...
                    [0, 0, np.sin(self.omega * self.dt), np.cos(self.omega * self.dt)],
                ],
                dtype=np.ndarray,
            )
        ...
        theta_next = (
            np.linalg.norm(v_k) * self.dt * np.tan(self.alpha) / self.d
        )  # Turn angle
    ...
    def one_step_algorithm_2(self, alpha_ref, v_ref, dt):
            ...
            A_k = np.array(
                [
                    [
                        1,
                        0,
                        np.sin(self.omega * dt) / self.omega,
                        -(1 - np.cos(self.omega * dt)) / self.omega,
                    ],
                    [
                        0,
                        1,
                    ...
                    [0, 0, np.sin(self.omega * dt), np.cos(self.omega * dt)],
                ],
                dtype=np.ndarray,
            )
        else:  # In case omega ~ 0
            A_k = np.array([[1, 0, dt, 0], [0, 1, 0, dt], [0, 0, 1, 0], [0, 0, 0, 1]])
        ...
        # Step 4: Update omega nad psi (based on theta)
        theta_next = (
            np.linalg.norm(v_k) * dt * np.tan(self.alpha) / self.d
        )  # Turn angle
        self.heading = self.heading + theta_next  # Heading in WCF
        self.omega = np.linalg.norm(v_k) * np.tan(self.alpha) / self.d
    ...
    def is_line_intersect_entity(
        self, line: np.array, vertices: List[np.array]
    ) -> bool:
        """
        PSEUDOCODE
    ...
    def find_intersection_ray_object(
        self, line: np.array, obj: Object, angle: float
    ) -> List:
        """
        PSEUDOCODE
        ...
        if self.is_line_intersect_entity(line, obj.vertices):
            for i in range(len(obj.lines)):
                if self.is_line_intersect_entity(line, obj.sides[i]):
                    ...
                    if (
                        0 < angle < np.pi / 2
                        and self.is_up(loc_point, cent)
                        and self.is_right(loc_point, cent)
                        or np.pi / 2 < angle < np.pi
                        and self.is_up(loc_point, cent)
                        and not self.is_right(loc_point, cent)
                        or np.pi < angle < 3 / 2 * np.pi
                        and not self.is_up(loc_point, cent)
                        and not self.is_right(loc_point, cent)
                        or 3 / 2 * np.pi < angle < 2 * np.pi
                        and not self.is_up(loc_point, cent)
                        and self.is_right(loc_point, cent)
                        or angle == 0
                        and self.is_right(loc_point, cent)
                        ...
                        dist.append(
                            self.dist_point_point(loc_point, self.position_center)
                        )

        if inters_points:  # if it not empty
            pos_min = dist.index(
                min(dist)
            )  # find position of the minimum distance in the list
    ...
    def find_intersection_line_ego(
        self, angle: float, length: float, width: float
    ) -> np.array:
        """
        PSEUDOCODE
    ...
    # MY own attempt #
    ##################
    def static_circogram_2(
        self, N: int, list_objects_simul, d_horizon: float, gausian_noise=None
    ):
        """
            ...
            for obj in list_objects_simul:
                for i in range(len(obj.lines)):
                    if self.is_line_intersect_entity(ray_line, obj.sides[i]):
                        loc_point = self.find_intersection_line_line(
                            ray_line, obj.lines[i]
                        )
                        distance = self.dist_point_point(
                            loc_point, self.position_center
                        )
                        ...
                        if (
                            self.dist_point_point(loc_point, P1[n]) < distance
                        ):  # Should be on correct side of object!
    ...
    def path_collision(self, p1, p2, obj: Object):
        ...
        for i in range(len(obj.lines)):
            if self.is_line_intersect_entity(trjactory_line, obj.sides[i]):
                ...
                intersection = self.find_intersection_line_line(
                    trjactory_line, obj.lines[i]
                )
                v1, v2 = obj.sides[i]
                # Check if intersection is between p1, p2
                if (
                    (p1[0] < intersection[0] and p2[0] > intersection[0])
                    ...
                    if (
                        (v1[0] < intersection[0] and v2[0] > intersection[0])
